...
async function handleRegister(request, env) {
  ...
  const { role, name, email, phone, password, country, city, postal_code, categories, native_language, additional_languages, work_type, company_name } = body;
  ...
  const result = await env.DB.prepare(
    'INSERT INTO users (role, name, email, phone, password_hash, country, city, postal_code, native_language, additional_languages, work_type, company_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  ).bind(role, name, email, phone || null, hash, country || null, city || null, postal_code || null, native_language || null, additional_languages ? JSON.stringify(additional_languages) : null, work_type || null, company_name || null).run();
...
async function handleLogin(request, env) {
  ...
  return json({
    ...
    user: {
      ...
      bio: user.bio, experience: user.experience, native_language: user.native_language, additional_languages: user.additional_languages ? JSON.parse(user.additional_languages) : [], avatar_url: user.avatar_url, work_type: user.work_type, company_name: user.company_name, postal_code: user.postal_code,
...
async function handleGetProfile(request, env, id) {
  ...
  if (user.role === 'master') {
    const mc = await env.DB.prepare("SELECT COUNT(*) as c FROM responses r JOIN orders o ON r.order_id = o.id WHERE r.master_id = ? AND r.status = 'accepted' AND o.status = 'completed'").bind(id).first();
    completedCount = mc?.c || 0;
  } else {
...
async function handleUpdateProfile(request, env) {
  ...
  const body = await request.json();
  const fields = ['name', 'phone', 'city', 'country', 'postal_code', 'bio', 'experience', 'email', 'native_language', 'avatar_url', 'work_type', 'company_name'];
  const updates = [];
  const values = [];
